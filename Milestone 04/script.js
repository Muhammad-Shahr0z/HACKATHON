...
function generateResume() {
    ...
    const profilePicture = document.getElementById('profile-picture-img').src;
    ...
    const experiences = Array.from(document.querySelectorAll('.experience-item')).map(item => {
        ...
        const companyName = item.querySelector('.company-name').value.trim();
        const jobDescription = item.querySelector('.job-description').value.trim();
        ...
        return `
            <div style="margin-bottom: 1rem;">
                <h3 style="font-size: 1.5rem; color: #007bff;">${toTitleCase(jobTitle)}</h3>
    ...
    const resumeContent = `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 800px; margin: auto;">
            ${profilePicture ? `<img src="${profilePicture}" style="border-radius: 50%; width: 150px; height: 150px; display: block; margin: 0 auto;">` : ''}
            ...
            <p style="font-size: 1.2rem; color: #333; margin: 0.5rem 0;">
                <strong>Email:</strong> <span style="color: #555;">${toTitleCase(email)}</span>
            </p>
            <p style="font-size: 1.2rem; color: #333; margin: 0.5rem 0;">
    ...
    if (resumeOutput && resumeContentDiv && downloadButton) {
        ...
        downloadButton.onclick = () => downloadResume(toTitleCase(name), resumeContent);
    }
}
